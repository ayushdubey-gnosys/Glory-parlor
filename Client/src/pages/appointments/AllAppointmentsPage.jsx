    ...
    <div className="min-h-screen bg-black text-white p-6">
      ...
      <div className="space-y-5">
        {appointments.map((appointment) => {
          ...
          return (
            <div key={appointment._id} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 md:p-6">
              <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-3 mt-2">
                    ...
                    <span className="px-4 py-2 rounded-xl bg-white text-black text-sm font-medium">{appointment.status}</span>

                    {(user?.role === "admin" || user?.role === "staff" || user?.role === "superadmin") && (
                      <div className="flex gap-2 mt-2">
                        {appointment.status === "unbooked" && (
                          <button onClick={async () => await updateMutation.mutateAsync({ id: appointment._id, data: { status: "booked" } })} className="px-3 py-1 bg-blue-600 text-white rounded">Mark Booked</button>
                        )}
                ...
                {canEdit && (
                  <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                    ...
                    <button onClick={() => deleteMutation.mutateAsync(appointment._id)} className="bg-red-500 text-white px-5 py-3 rounded-2xl font-medium w-full">Delete</button>
                  </div>
                )}
              </div>
            </div>
      ...
      {meta && (
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-4 py-2 rounded border bg-zinc-900 disabled:opacity-50">Prev</button>
            <button disabled={page >= meta.totalPages} onClick={() => setPage((p) => Math.min(meta.totalPages, p + 1))} className="px-4 py-2 rounded border bg-zinc-900 disabled:opacity-50">Next</button>
          </div>
        </div>
      )}
    </div>
